...
function App() {
  ...
  async function importRecommendations() {
    ...
    try {
      ...
      setBanner({
        type: "success",
        text: `已导入 ${response.processed_items.length} 条推荐文献。`,
      });
    } catch (error) {
  ...
  async function generateOutlinePrompt() {
    ...
    try {
      ...
      setBanner({ type: "success", text: "大纲 Prompt 已生成，请发送给通用 LLM。" });
    } catch (error) {
  ...
  async function importOutline() {
    ...
    try {
      ...
      setBanner({ type: "success", text: "大纲已导入，请确认无误后保存。" });
    } catch (error) {
  ...
  async function saveOutlineConfirmation() {
    ...
    try {
      ...
      syncCurrentProject(projectResponseFromState(response));
      await refreshProjectList();
      setBanner({ type: "success", text: "大纲已确认保存。" });
    } catch (error) {
      setBanner({ type: "error", text: getErrorMessage(error, "保存大纲失败。") });
    } finally {
      setBusyLabel("");
    }
  }
  ...
  async function generateChunkPlanPrompt() {
    ...
    try {
      ...
      setBanner({ type: "success", text: "切块规划 Prompt 已生成，请发送给通用 LLM。" });
    } catch (error) {
      setBanner({ type: "error", text: getErrorMessage(error, "生成切块规划 Prompt 失败。") });
    } finally {
      setBusyLabel("");
    }
  }
  ...
  async function importChunkPlan() {
    ...
    try {
      ...
      setBanner({ type: "success", text: "切块规划已导入，请确认无误后保存。" });
    } catch (error) {
  ...
  async function saveChunkPlanConfirmation() {
    ...
    try {
      ...
      syncCurrentProject(projectResponseFromState(response));
      await refreshProjectList();
      setBanner({ type: "success", text: "切块规划已确认保存。" });
    } catch (error) {
      setBanner({ type: "error", text: getErrorMessage(error, "保存切块规划失败。") });
    } finally {
      setBusyLabel("");
    }
  }
  ...
  async function generateBlockPrompt() {
    ...
    setBusyLabel("正在生成正文块 Prompt");
    setBanner(null);
    try {
      const response = await window.qnuCopilot.getBlockPrompt(currentProject.project_id);
      startTransition(() => {
        setBlockPrompt(response);
      });
      setBanner({ type: "success", text: "正文块 Prompt 已生成，请发送给通用 LLM。" });
    } catch (error) {
  ...
  async function importBlockContent() {
    ...
    if (!blockRawText.trim()) {
      setBanner({ type: "error", text: "请先粘贴正文块 JSON。" });
      return;
    }
    setBusyLabel("正在导入正文块 JSON");
    setBanner(null);
    try {
      ...
      setBanner({ type: "success", text: "正文块已导入。" });
    } catch (error) {
  ...
  async function generateCompressPrompt() {
    ...
    setBusyLabel("正在生成压缩上下文 Prompt");
    setBanner(null);
    try {
      const response = await window.qnuCopilot.getCompressPrompt(currentProject.project_id);
      startTransition(() => {
        setCompressPrompt(response);
      });
      setBanner({ type: "success", text: "压缩上下文 Prompt 已生成，请发送给通用 LLM。" });
    } catch (error) {
      setBanner({ type: "error", text: getErrorMessage(error, "生成压缩上下文 Prompt 失败。") });
    } finally {
      setBusyLabel("");
    }
  }
  ...
  async function importCompressedContext() {
    ...
    try {
      ...
      setBanner({ type: "success", text: "压缩上下文已导入。" });
    } catch (error) {
  ...
  async function importSingleReference(sourceIndex: number) {
    ...
    setBusyLabel(`正在导入第 ${sourceIndex} 篇文献`);
    setBanner(null);
    try {
      const response = await window.qnuCopilot.importSingleReference(
        currentProject.project_id,
        sourceIndex,
      );
      syncCurrentProject(projectResponseFromState(response));
      await refreshProjectList();
      setBanner({ type: "success", text: `第 ${sourceIndex} 篇文献已导入。` });
    } catch (error) {
      setBanner({ type: "error", text: getErrorMessage(error, "导入文献失败。") });
    } finally {
      setBusyLabel("");
    }
  }
  ...
  async function skipReference(sourceIndex: number, reason: "unavailable" | "user_choice") {
    ...
    setBusyLabel(`正在标记第 ${sourceIndex} 篇文献`);
    setBanner(null);
    try {
      const response = await window.qnuCopilot.skipReference(
        currentProject.project_id,
        sourceIndex,
        reason,
      );
    ...
    <div className="app-shell">
      ...
      <main className="landing-layout">
        <section className="card primary-card">
          <div className="section-head">
            <div>
              <p className="section-kicker">创建项目</p>
              <h2>开始新项目</h2>
            </div>
          </div>
        ...
        <section className="card primary-card">
          <div className="section-head">
            <div>
              <p className="section-kicker">载入项目</p>
              <h2>继续现有项目</h2>
            </div>
          </div>
      ...
      <main className="working-layout">
        ...
        <section className="card wide-card">
          <div className="section-head">
            <div>
              <p className="section-kicker">最终步骤</p>
              <h2>导出论文文档</h2>
            </div>
            <div className="mini-actions">
              {lastDocxPath ? (
